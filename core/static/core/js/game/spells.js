...
export function normalizeSpell(spell = {}, cardSlug = 'card', index = 0) {
  ...
  const inferredType = /fusi[oó]n/i.test(spell.name || '') ? 'fusion'
    : /evoluci[oó]n/i.test(spell.name || '') ? 'evolution'
      : /invoc/i.test(text) ? 'summon' : 'damage';
  ...
  return {
    ...
    damage: spell.damage || { min: Number(spell.damage_min) || 0, max: Number(spell.damage_max) || 0 },
...
export function normalizeSpells(card = {}) {
  ...
  return (Array.isArray(card.spells) ? card.spells : []).map((spell, index) => normalizeSpell(spell, cardSlug, index));
}
