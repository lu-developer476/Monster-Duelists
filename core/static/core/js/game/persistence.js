...
export function validateSavePayload(payload) {
  if (!payload || typeof payload !== "object")
    return "La partida guardada no es un objeto válido.";
  if (!Number.isInteger(payload.schemaVersion))
    return "La partida guardada no indica su versión.";
  if (payload.schemaVersion > SCHEMA_VERSION)
    return "La partida fue creada por una versión más nueva del juego.";
  if (
    !payload.match ||
    ...
    return "La partida guardada está incompleta.";
  if (!payload.matchConfig || typeof payload.matchConfig !== "object")
    return "La partida guardada no contiene su configuración.";
  return null;
}
  ...
  if (payload?.match && payload.schemaVersion == null) {
    return {
      ...
      matchConfig: normalizeMatchConfig(
        payload.matchConfig || {
          initialHandSize: payload.match.initial_hand_size || 2,
        },
      ),
    };
  }
...
export function restoreMatch(storage) {
  ...
  try {
    ...
    return {
      ok: false,
      reason: "corrupt",
      message: "La partida guardada estaba dañada y fue descartada.",
    };
  }
  const error = validateSavePayload(payload);
  if (error) {
    storage.removeItem(STORAGE_KEY);
    return { ok: false, reason: "invalid", message: error };
  }
